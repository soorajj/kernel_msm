...
static int __init cpufreq_read_arg_max_oc0(char *max_oc0)
{
	...
	if (err) {
		arg_max_oc0 = 2265600;
		printk(KERN_INFO "[Sooplus]: max_oc0='%i'\n", arg_max_oc0);
		return 1;
	}
...
static int __init cpufreq_read_arg_max_oc1(char *max_oc1)
{
	...
	if (err) {
		arg_max_oc1 = 2265600;
		printk(KERN_INFO "[Sooplus]: max_oc1='%i'\n", arg_max_oc1);
		return 1;
	}
...
static int __init cpufreq_read_arg_max_oc2(char *max_oc2)
{
	...
	if (err) {
		arg_max_oc2 = 2265600;
		printk(KERN_INFO "[Sooplus]: max_oc2='%i'\n", arg_max_oc2);
		return 1;
	}
...
static int __init cpufreq_read_arg_max_oc3(char *max_oc3)
{
	...
	if (err) {
		arg_max_oc3 = 2265600;
		printk(KERN_INFO "[Sooplus]: max_oc3='%i'\n", arg_max_oc3);
		return 1;
	}
...
static void apply_undervolting(void)
{
	if (uv_bin == 6) {
		drv.acpu_freq_tbl[0].vdd_core = 625000;
	        printk(KERN_INFO "[Sooplus]: min_voltage='%i'\n", drv.acpu_freq_tbl[0].vdd_core );
	}
